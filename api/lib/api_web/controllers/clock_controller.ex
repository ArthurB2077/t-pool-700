    ...
    if(conn.assigns == %{}) do
      ...
      if(id !== nil) do
        ...
        if (clock.status === false) do
          ...
          |> Repo.update()
          newClock = %{ id: clock.id, time: clock.time, status: true , user: clock.user}
          conn |> render(ApiWeb.WorkingTimesView, "working_time_view.json", %{status: 200, success: true, message: "Working time started for this user", content: newClock})
        else
          ...
          newClock = %{ id: clock.id, time: clock.time, status: false , user: clock.user}
          conn |> render(ApiWeb.WorkingTimesView, "working_time_view.json", %{status: 200, success: true, message: "Working time stopped for this user", content: newClock})
        end
      else
