defmodule ApiWeb.TeamMemberController do
  ...
  def addteammember(conn, params) do
    if(conn.assigns == %{}) do
      ...
      if (user !== nil && team !== nil) do
        Repo.insert(%Member{team: team.id, user: user.id})
        conn |> render(ApiWeb.TeamsMemberView, "add_team_member.json", status: 201, success: true, message: "Member added to team", userid: user.id)
      else
        conn |> render(ApiWeb.ErrorView, "error.json", status: 403, error: "Failed to add member to team")
      end
    end
  end
  ...
  def retrieve(conn, params) do
    if(conn.assigns == %{}) do
      ...
      if (retrieved !== nil && retrieved !== []) do
        ...
        conn |> render(ApiWeb.ErrorView, "error.json", %{status: 403, error: "No member found"})
      end
    end
  end
  ...
  def delete(conn, params) do
    if(conn.assigns == %{}) do
      ...
      if(userID !== nil && teamID !== nil) do
        ...
        if(member !== nil) do
          Repo.delete(member)
          conn |> render(ApiWeb.UserView, "user_view.json", %{status: 204, success: true, message: "Member remove from team", content: member})
        else
          conn |> render(ApiWeb.ErrorView, "error.json", %{status: 403, error: "No member found"})
        end
      else
