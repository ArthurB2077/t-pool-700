<template>
  <div class="navbar" role="navigation">
    <div class="navbar-logo-container">
      <img class="navbar-logo" src="../assets/tm_logo_lighter.png" />
    </div>

    <div class="navbar-menu-container">
      <ul v-if="session && session.active" class="navbar-menu-list">
        <li
          :class="`navbar-menu-item ${
            $route && $route.path.includes('home')
              ? 'navbar-menu-item__selected'
              : ''
          }`"
        >
          <RouterLink to="/home">
            <svg viewBox="0 0 24 24">
              <path
                clip-rule="evenodd"
                d="m14.0138 4.04532c-1.1807-.93217-2.8469-.93217-4.02764 0l-5.5 4.34211c-.78069.61633-1.23616 1.55622-1.23616 2.55087v6.5616c0 1.7949 1.45507 3.25 3.25 3.25h1.5c.13807 0 .25-.1119.25-.25v-3c0-1.5188 1.23122-2.75 2.75-2.75h2c1.5188 0 2.75 1.2312 2.75 2.75v3c0 .1381.1119.25.25.25h1.5c1.7949 0 3.25-1.4551 3.25-3.25v-6.5616c0-.99465-.4555-1.93454-1.2362-2.55087zm-4.95711-1.17732c1.72571-1.36241 4.16091-1.36241 5.88661 0l5.5 4.34211c1.141.90079 1.8067 2.27447 1.8067 3.72819v6.5616c0 2.6233-2.1266 4.75-4.75 4.75h-1.5c-.9665 0-1.75-.7835-1.75-1.75v-3c0-.6904-.5596-1.25-1.25-1.25h-2c-.6904 0-1.25.5596-1.25 1.25v3c0 .9665-.7835 1.75-1.75 1.75h-1.5c-2.62335 0-4.75-2.1267-4.75-4.75v-6.5616c0-1.45372.66569-2.8274 1.80669-3.7282z"
                fill-rule="evenodd"
              />
            </svg>
          </RouterLink>
        </li>
        <li
          :class="`navbar-menu-item ${
            $route && $route.path.includes('working_times')
              ? 'navbar-menu-item__selected'
              : ''
          }`"
        >
          <RouterLink :to="'/working_times/' + session.id">
            <svg viewBox="0 0 512 512">
              <path
                d="m316.695 512h-121.39v-53.312c-13.761-4.112-27.047-9.62-39.699-16.456l-37.707 37.706-85.835-85.836 37.705-37.707c-6.836-12.652-12.344-25.938-16.456-39.699h-53.313v-121.391h53.312c4.112-13.761 9.62-27.047 16.456-39.699l-37.705-37.707 85.835-85.836 37.707 37.706c12.652-6.836 25.938-12.344 39.699-16.456v-53.313h121.391v53.312c13.761 4.112 27.047 9.62 39.699 16.456l37.707-37.706 85.836 85.836-37.706 37.707c6.836 12.652 12.344 25.938 16.456 39.699h53.313v121.391h-53.312c-4.112 13.761-9.62 27.047-16.456 39.699l37.706 37.707-85.836 85.836-37.707-37.706c-12.652 6.836-25.938 12.344-39.699 16.456v53.313zm-91.39-30h61.391v-46.66l11.525-2.744c18.654-4.442 36.413-11.804 52.784-21.88l10.091-6.211 33.006 33.007 43.41-43.41-33.007-33.006 6.211-10.091c10.076-16.371 17.438-34.13 21.88-52.784l2.744-11.525h46.66v-61.391h-46.66l-2.744-11.525c-4.442-18.654-11.804-36.413-21.88-52.784l-6.211-10.091 33.007-33.006-43.41-43.41-33.006 33.007-10.091-6.211c-16.371-10.076-34.13-17.438-52.784-21.88l-11.525-2.744v-46.661h-61.391v46.66l-11.525 2.744c-18.654 4.442-36.413 11.804-52.784 21.88l-10.091 6.211-33.006-33.007-43.409 43.41 33.005 33.006-6.21 10.091c-10.076 16.371-17.438 34.13-21.88 52.784l-2.744 11.525h-46.661v61.391h46.66l2.744 11.525c4.442 18.654 11.804 36.413 21.88 52.784l6.21 10.091-33.005 33.006 43.409 43.41 33.006-33.007 10.091 6.211c16.371 10.076 34.13 17.438 52.784 21.88l11.525 2.744v46.661z"
              />
              <path
                d="m256 399.075c-80.227 0-145.496-65.269-145.496-145.495s65.269-145.496 145.496-145.496 145.496 65.27 145.496 145.496-65.269 145.495-145.496 145.495zm0-260.991c-63.685 0-115.496 51.812-115.496 115.496s51.811 115.495 115.496 115.495 115.496-51.811 115.496-115.495-51.811-115.496-115.496-115.496z"
              />
              <path d="m316 268.58h-75v-75h30v45h45z" />
            </svg>
          </RouterLink>
        </li>
        <li
          :class="`navbar-menu-item ${
            $route && $route.path.includes('chart_manager')
              ? 'navbar-menu-item__selected'
              : ''
          }`"
        >
          <RouterLink :to="'/chart_manager/' + session.id">
            <svg viewBox="0 0 32 32">
              <g id="Layer_7" data-name="Layer 7">
                <path
                  d="m31.9336 19.4463a16.0029 16.0029 0 0 0 -15.9385-15.3672c-.1269 0-.2549.0015-.3818.0049a16.0378 16.0378 0 0 0 -15.5576 15.9394 15.7648 15.7648 0 0 0 1.8076 7.36 1 1 0 0 0 .8867.5376h26.5a.9988.9988 0 0 0 .8867-.5376 15.9623 15.9623 0 0 0 1.7969-7.9371zm-3.3047 6.4746h-25.2589a13.7635 13.7635 0 0 1 -1.3144-5.8975 14.0264 14.0264 0 0 1 13.6044-13.9404c.1064-.0024.2119-.0034.3173-.0034a14.1766 14.1766 0 0 1 13.958 13.438 13.9591 13.9591 0 0 1 -1.3064 6.4033z"
                />
                <path
                  d="m22.0566 11.5952-8.6211 4.9258a3.6953 3.6953 0 0 0 -.8564 5.916 3.697 3.697 0 0 0 5.916-.8574l4.9258-8.6196a1 1 0 0 0 -1.3643-1.3643zm-5.2978 8.9922a1.6993 1.6993 0 0 1 -2.7656.4355 1.698 1.698 0 0 1 .4345-2.7652l5.4375-3.1077z"
                />
              </g>
            </svg>
          </RouterLink>
        </li>
        <li
          :class="`navbar-menu-item ${
            $route && $route.path.includes('users')
              ? 'navbar-menu-item__selected'
              : ''
          }`"
        >
          <RouterLink :to="'/users/' + session.id">
            <svg viewBox="-42 0 512 512.001">
              <path
                d="m210.351562 246.632812c33.882813 0 63.21875-12.152343 87.195313-36.128906 23.96875-23.972656 36.125-53.304687 36.125-87.191406 0-33.875-12.152344-63.210938-36.128906-87.191406-23.976563-23.96875-53.3125-36.121094-87.191407-36.121094-33.886718 0-63.21875 12.152344-87.191406 36.125s-36.128906 53.308594-36.128906 87.1875c0 33.886719 12.15625 63.222656 36.128906 87.195312 23.980469 23.96875 53.316406 36.125 87.191406 36.125zm-65.972656-189.292968c18.394532-18.394532 39.972656-27.335938 65.972656-27.335938 25.996094 0 47.578126 8.941406 65.976563 27.335938 18.394531 18.398437 27.339844 39.980468 27.339844 65.972656 0 26-8.945313 47.578125-27.339844 65.976562-18.398437 18.398438-39.980469 27.339844-65.976563 27.339844-25.992187 0-47.570312-8.945312-65.972656-27.339844-18.398437-18.394531-27.34375-39.976562-27.34375-65.976562 0-25.992188 8.945313-47.574219 27.34375-65.972656zm0 0"
              />
              <path
                d="m426.128906 393.703125c-.691406-9.976563-2.089844-20.859375-4.148437-32.351563-2.078125-11.578124-4.753907-22.523437-7.957031-32.527343-3.3125-10.339844-7.808594-20.550781-13.375-30.335938-5.769532-10.15625-12.550782-19-20.160157-26.277343-7.957031-7.613282-17.699219-13.734376-28.964843-18.199219-11.226563-4.441407-23.667969-6.691407-36.976563-6.691407-5.226563 0-10.28125 2.144532-20.042969 8.5-6.007812 3.917969-13.035156 8.449219-20.878906 13.460938-6.707031 4.273438-15.792969 8.277344-27.015625 11.902344-10.949219 3.542968-22.066406 5.339844-33.042969 5.339844-10.96875 0-22.085937-1.796876-33.042968-5.339844-11.210938-3.621094-20.300782-7.625-26.996094-11.898438-7.769532-4.964844-14.800782-9.496094-20.898438-13.46875-9.753906-6.355468-14.808594-8.5-20.035156-8.5-13.3125 0-25.75 2.253906-36.972656 6.699219-11.257813 4.457031-21.003906 10.578125-28.96875 18.199219-7.609375 7.28125-14.390625 16.121094-20.15625 26.273437-5.558594 9.785157-10.058594 19.992188-13.371094 30.339844-3.199219 10.003906-5.875 20.945313-7.953125 32.523437-2.0625 11.476563-3.457031 22.363282-4.148437 32.363282-.679688 9.777344-1.023438 19.953125-1.023438 30.234375 0 26.726562 8.496094 48.363281 25.25 64.320312 16.546875 15.746094 38.4375 23.730469 65.066406 23.730469h246.53125c26.621094 0 48.511719-7.984375 65.0625-23.730469 16.757813-15.945312 25.253906-37.589843 25.253906-64.324219-.003906-10.316406-.351562-20.492187-1.035156-30.242187zm-44.90625 72.828125c-10.933594 10.40625-25.449218 15.464844-44.378906 15.464844h-246.527344c-18.933594 0-33.449218-5.058594-44.378906-15.460938-10.722656-10.207031-15.933594-24.140625-15.933594-42.585937 0-9.59375.316406-19.066407.949219-28.160157.617187-8.921874 1.878906-18.722656 3.75-29.136718 1.847656-10.285156 4.199219-19.9375 6.996094-28.675782 2.683593-8.378906 6.34375-16.675781 10.882812-24.667968 4.332031-7.617188 9.316407-14.152344 14.816407-19.417969 5.144531-4.925781 11.628906-8.957031 19.269531-11.980469 7.066406-2.796875 15.007812-4.328125 23.628906-4.558594 1.050781.558594 2.921875 1.625 5.953125 3.601563 6.167969 4.019531 13.277344 8.605469 21.136719 13.625 8.859375 5.648437 20.273437 10.75 33.910156 15.152344 13.941406 4.507812 28.160156 6.796875 42.273437 6.796875 14.113282 0 28.335938-2.289063 42.269532-6.792969 13.648437-4.410156 25.058594-9.507813 33.929687-15.164063 8.042969-5.140624 14.953125-9.59375 21.121094-13.617187 3.03125-1.972656 4.902344-3.042969 5.953125-3.601563 8.625.230469 16.566406 1.761719 23.636719 4.558594 7.636719 3.023438 14.121093 7.058594 19.265625 11.980469 5.5 5.261719 10.484375 11.796875 14.816406 19.421875 4.542969 7.988281 8.207031 16.289062 10.886719 24.660156 2.800781 8.75 5.15625 18.398438 7 28.675782 1.867187 10.433593 3.132812 20.238281 3.75 29.144531v.007812c.636719 9.058594.957031 18.527344.960937 28.148438-.003906 18.449219-5.214844 32.378906-15.9375 42.582031zm0 0"
              />
            </svg>
          </RouterLink>
        </li>
      </ul>
    </div>

    <div
      v-if="session && session.active"
      class="navbar-logout-container"
      role="link"
      v-on:click="unregisterUser()"
    >
      <svg viewBox="0 0 512 512" data-name="Layer 1">
        <path
          d="m363.335 488a24 24 0 0 1 -24 24h-226.253a80.09 80.09 0 0 1 -80-80v-352a80.09 80.09 0 0 1 80-80h226.253a24 24 0 0 1 0 48h-226.253a32.035 32.035 0 0 0 -32 32v352a32.034 32.034 0 0 0 32 32h226.253a24 24 0 0 1 24 24zm108.553-248.97-114.051-114.052a24 24 0 1 0 -33.937 33.941l73.077 73.081h-188.936a24 24 0 1 0 0 48h188.935l-73.08 73.08a24 24 0 1 0 33.941 33.941l114.051-114.05a24 24 0 0 0 0-33.941z"
        />
      </svg>
    </div>
  </div>
</template>

<script>
import { RouterLink } from "vue-router";
import router from "../router";

export default {
  data() {
    return {
      user: {
        username: "",
        id: "",
        email: "",
      },
      session: {
        active: false,
        id: null,
        username: "",
        email: "",
      },
    };
  },
  created() {
    if (!localStorage.session) {
      localStorage.session = JSON.stringify({
        ...this.session,
      });
    } else {
      this.session = JSON.parse(localStorage.session);
    }
  },
  methods: {
    unregisterUser: async function () {
      const newSession = {
        active: false,
        id: null,
        username: "",
        email: "",
        isAuthoriseAdmin: false,
        isAuthoriseManager: false,
        token: "",
      };
      const response = await fetch(process.env.VUE_APP_API_URL + "/logout", {
        mode: "cors",
        method: "DELETE",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
          Authorization: JSON.parse(localStorage.session).token,
        },
      });
      const {
        status: status,
        success: success,
        message: message,
        error: error,
      } = await response.json();
      if (success) {
        this.$toast.success(message, { position: "top-right" });
      } else {
        this.$toast.error(error, { position: "top-right" });
      }
      localStorage.session = JSON.stringify(newSession);
      this.session = newSession;
      router.push("/");
    },
  },
  watch: {
    $route(to, from) {
      if (from.name === "login" && to.name === "home") {
        this.session = JSON.parse(localStorage.session);
      }
    },
  },
};
</script>

<style scoped lang="scss">
.navbar {
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: column;
  margin: 0;
  background-color: var(--bg-1);

  .navbar-logo-container {
    width: 100%;
    height: 20%;
    display: flex;
    justify-content: center;
    align-items: flex-start;
    margin-top: 30px;

    .navbar-logo {
      width: 78px;
    }
  }

  .navbar-menu-container {
    width: 100%;
    height: 60%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;

    .navbar-menu-list {
      height: 100%;
      display: flex;
      flex-direction: column;
      justify-content: space-between;

      .navbar-menu-item {
        svg {
          width: 36px;
          fill: var(--color-2);
          transition: transform 0.2s;

          /* Animation */
          &:hover {
            transform: scale(1.25);
          }
        }

        &__selected {
          svg {
            fill: var(--color-1);
          }
        }
      }
    }
  }

  .navbar-logout-container {
    width: 100%;
    height: 20%;
    display: flex;
    justify-content: center;
    align-items: flex-end;
    margin-bottom: 30px;

    svg {
      width: 34px;
      fill: var(--color-2);
      transition: transform 0.2s;

      /* Animation */
      &:hover {
        transform: scale(1.25);
      }
    }

    &:hover {
      cursor: pointer;
    }
  }
}

@media screen and (min-width: 150px) and (max-width: 550px) {
  .navbar {
    background-color: var(--bg-4);
    flex-direction: row;
    display: flex;
    align-items: center;

    .navbar-logo-container {
      display: none;

      .navbar-logo {
        display: none;
      }
    }

    .navbar-menu-container {
      width: 100% !important;
      height: 100%;
      display: flex;
      flex-direction: row;
      justify-content: inherit;
      align-items: center;

      .navbar-menu-list {
        height: 100%;
        display: flex;
        flex-direction: row;
        justify-content: space-between;

        .navbar-menu-item {
          display: flex;
          align-items: center;
          justify-content: center;
          width: 20vw;

          svg {
            width: 36px;
            fill: var(--color-2);
            transition: inherit;

            &:hover {
              transform: inherit;
            }
          }

          &__selected {
            svg {
              fill: var(--color-1);
            }
          }
        }
      }
    }

    .navbar-logout-container {
      width: 21vw;
      height: 100%;
      display: flex;
      justify-content: center;
      align-items: center;
      margin-bottom: 2.5px;

      svg {
        width: 34px;
        fill: var(--color-2);
        transition: inherit;

        &:hover {
          transform: inherit;
        }
      }

      &:hover {
        cursor: inherit;
      }
    }
  }
}
</style>
