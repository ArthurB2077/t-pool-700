export default {
  ...
  getUserByEmailAndUsername: async function (email, username) {
    const response = await fetch(
      `${process.env.VUE_APP_API_URL}/users?email=${email}&username=${username}`,
      {
        mode: "cors",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
  ...
  getUsersInTeam: async function (teamID) {
    const response = await fetch(
      `${process.env.VUE_APP_API_URL}/teamMember/${teamID}`,
      {
        mode: "cors",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
  ...
  createTeam: async function (userID, newTeam) {
    const response = await fetch(
      `${process.env.VUE_APP_API_URL}/teams/${userID}`,
      {
        method: "POST",
        mode: "cors",
        headers: {
  ...
  addMemberInTeam: async function (userID, teamID) {
    const response = await fetch(
      `${process.env.VUE_APP_API_URL}/teamMember/${userID}/${teamID}`,
      {
        method: "POST",
        mode: "cors",
        headers: {
